...
class SimpleBot(ircbot.SingleServerIRCBot):
	...
	def __init__(self, bd, configfilename):
		...
		self._set_defaults()
		self._read_config(configfilename)
	...
	def _read_config(self, filename):
		...
		bot = doc.getElementsByTagName("bot")[0]
	...
	def on_pubmsg(self, c, e):
		...
		if len(a) > 1 and a[0].lower() == self.connection.get_nickname().lower():
			self.do_command(c, irclib.nm_to_n(e.source()), e.target(), a[1].strip())
	...
	def on_privmsg(self, c, e):
		...
		self.do_command(c, irclib.nm_to_n(e.source()), e.target(), e.arguments()[0])
