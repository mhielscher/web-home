...
/*
GameMain - Handles the GUI and game windows
Vehicle - The player vehicle
Projectile - What the vehicles shoot
EnvironmentDefinition - Modifiers to vehicle and proj. behavior, i.e. friction and air resistance
-obs- PlayerInputHandler - handles the keyboard inputs for the local player(s)
	Arena now handles this directly
NetworkInputHandler - handles the network inputs from the server describing remote player(s)
NetworkOutputHandler - handles the network outputs to the server describing local player(s)
Arena - draws the vehicles and projectiles, and houses barriers and items
Barrier - a collection of arbitrarily rotated rectangles defining a solid barrier
Item - an item for the vehicles, e.g. ammunition, upgrades, bonuses, etc.
*/
...
/*	protected ResourceBundle resbundle;
	...
	public GameMain() {
		...
		String address = JOptionPane.showInputDialog("Enter server IP:");
		Socket server = null;
		try {
			server = new Socket(address, 2829);
		} catch (Exception e) {
			System.err.println(e);
		}
		
		gameSpace = new Arena(new NetworkInputHandler(server), new NetworkOutputHandler(server));
		gameSpace.setSize(700, 500);
		gameSpace.setLocation(50, 50);
		//gameSpace.placeNewVehicle("Me");
		
		this.getContentPane().setLayout(null);
		this.getContentPane().add(gameSpace);
		addKeyListener(gameSpace.newListener());
	...
	public void initOnMac() {
		...
		resbundle = ResourceBundle.getBundle ("GameMainstrings", Locale.getDefault());
